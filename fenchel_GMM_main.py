...
def main(args):
    ...
    if args.classification_model == 'LogisticRegression':
        classification_model = LogisticRegression(args._input_dim, args._num_class).to('cuda')
    else:
        raise NotImplementedError()
    
    if args.influence_model == 'Linear_IF':
        influence_model = Linear_IF(args._input_dim, args._num_class).to('cuda')
    else:
        raise NotImplementedError()
    ...
    for epoch in range(args.max_epoch):
        ...
        #                             "x", "class")})
        wandb.log({'total_weight_variance': torch.var(fenchen_classifier._weights).item()})
        ...
        helpful_data_y = [ train_dataset[x][1].item() for x in helpful[:num_to_plot]]
        ...
        harmful_data_y = [ train_dataset[x][1].item() for x in harmful[:num_to_plot]]
