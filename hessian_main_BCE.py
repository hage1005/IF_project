...
def main(args):
    ...
    train_dataset, train_dataset_no_transform = ImageDataset.get_train(label_map, torch.FloatTensor)
    dev_dataset, dev_dataset_no_transform = ImageDataset.get_dev(label_map)
    ...
    hessian_solver = hessianSolver(classification_model, pretrain_ckpt_path, inv_hessian_path, criterion)
    ...
    def loss_grad_at_point(model, x, y):
        ...
        out = model(x)
        loss = criterion(out.view(-1), y.float().view(-1))
        loss.backward()
