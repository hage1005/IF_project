...
def main(args):
    ...
    x_test, y_test = test_loader.collate_fn(
        [x_test]), test_loader.collate_fn(
        [y_test])

    if args.method == 'tracIn':
        influences, harmful, helpful = tracin_cp(
            args.ckpt_dir, x_test, y_test, train_loader, device)
    elif args.method == 'hvp':
        influences, harmful, helpful = calc_influence_single(
            args.ckpt_dir, x_test, y_test, train_loader, device, args.recursion_depth, args.r_averaging)
    ...
    json_path = os.path.join(
        args.output_path,
        args.method +
        f"_test_id_{args.test_id_num}.json")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="fenchel",
        help="use tracIn or hvp or fenchel")
    parser.add_argument(
        '--batch_size',
        type=int,
        default=64,
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="id of test example in testloader")
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="1 is the default value from repo")
    parser.add_argument(
        '--recursion_depth',
        type=int,
        default=50000,
