...
def get_cifar10_train(classes, num_per_class = None):
    ...
    train_data_no_transform_raw = torchvision.datasets.CIFAR10(root='../data', train=True,
                                            download=True, transform=transforms.ToTensor())

    train_data_no_transform_sampled = _subsample_by_classes(train_data_no_transform_raw, classes, num_per_class)

    train_data_x_no_transform = torch.stack([example[0] for example in train_data_no_transform_sampled])
    train_data_x_transformed = torch.stack([trans(x) for x in train_data_x_no_transform])
    train_data_y = torch.LongTensor([example[1] for example in train_data_no_transform_sampled])
    ...
    train_data_no_transform_tensor = TensorDataset(train_data_x_no_transform, train_data_y)
    print('loaded train_dataset with {} samples'.format(len(train_data_no_transform_sampled) ))
...
def get_cifar10_test():
    ...
    test_data_no_transform = FolderDataset("data/cifar10/test/original_sampled")
